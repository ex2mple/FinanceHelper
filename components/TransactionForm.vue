...
import {transactionSchema} from "~/types/transactionModel";
...
const emit = defineEmits(['closeForm', 'mutateData'])
...
const onSubmit = async (data: any) => {
  ...
  if (props.transaction) {
    try {
      const response = await instance.patch('/transactions/' + props.transaction.id, newData)
      toast.add({
        severity: 'success',
        summary: 'Готово',
        detail: 'Транзакция успешно обновлена',
        life: 3000,
      })
      emit('closeForm')
      emit('mutateData', response.data)
    } catch (error: any) {
      toast.add({
        severity: 'error',
        summary: 'Ошибка',
